				...
				<div class="row inputForm">
					...
					<div class="row signupRow">
						...
						<div class="col-5 signupInput">
							<input type="text" class="form-control form-control-sm" id="id" name="id" check_result="fail" maxlength="20">
						</div>
						<div class="col-4 signupSide">
							<button type="button" id="checkBtn" class="btn btn-outline-warning btn-sm">중복확인</button>&nbsp;
					...
					<div class="row signupRow">
						...
						<div class="col-5 signupInput">
							<input type="password" class="form-control form-control-sm" id="pw" name="pw" check_result="fail" maxlength="20">
						</div>
						<div class="col-4 signupSide">
					...
					<div class="row signupRow">
						<div class="col-2 signupTitle">*비밀번호 확인</div>
						<div class="col-5 signupInput">
							<input type="password" class="form-control form-control-sm" id="pwChk" maxlength="20">
						</div>
						<div class="col-5 signupSide">
							<span id="pwChkNotice"></span>
						</div>
					</div>
					...
					<div class="row signupRow">
						...
						<div class="col-5 signupInput">
							<input type="text" class="form-control form-control-sm" id="email" name="email" check_result="fail" maxlength="30"> 
							<input type="text"	class="form-control form-control-sm emailChk" id="emailChk" placeholder="인증번호 입력" disabled="disabled" maxlength="8">
						</div>
						<div class="col-5">	
							<div class="row-8 signupSide" style="padding-top:0px;">
								<button type="button" id="emailChkBtn" class="btn btn-outline-warning btn-sm">메일인증</button>
							</div>	
							<div class="row-8 signupSide" style="margin-top:10px;">				
								<span id="emailChkNotice"></span>		
							</div>	
						</div>
					</div>
...
정보주체는 개인정보 수집에 동의를 거부할 권리가 있습니다. 다만, 필수 항목에 대한 동의를 거부할 시 저희가 제공하는 서비스를 이용할 수 없습니다. </textarea>
				...
				<div class="row buttonBox">
					...
					<div class="col">
						<button type="button" id="submitBtn" class="btn btn btn-dark">가입</button>
					</div>
				</div>
	...
	<script>
		...
		$("#emailChkBtn").on("click",function() {
			...
			$.ajax({
				type : "GET",
				url : "${pageContext.request.contextPath}/member/checkEmail?email="	+ email,
				
				success:function(data){  
					//메일발송 성공하면 인증번호 입력창 열기
		            emailChk.attr("disabled",false); 
		            key = data;
		            alert("인증번호를 "+email+" 으로 발송하였습니다.");
		...
		$(function() {
			$('#emailChk').change(function() {
				if ($("#emailChk").val() !== "") {
					   if($("#emailChk").val() == key){  // 일치할 경우
						   $('#emailChkNotice').html('인증번호 일치');
							$('#emailChkNotice').css('color','green'); 
							$("#email").attr("check_result","success"); 
							
					    } else { // 일치하지 않을 경우
					    	$('#emailChkNotice').html('인증번호 불일치');
							$('#emailChkNotice').css('color','red');
		...
		$("#checkBtn").on("click", function() {
			...
			//아이디 정규식 검사
			let regexId = /^[a-z0-9]{6,20}$/;
			if (!regexId.test(id)) {
				alert("아이디는 영문 소문자와 숫자 6~20자리로 입력해야합니다.");
				$("#id").val("");
				$("#id").focus();
				return false;
			}
			...
			$('#pw, #pwChk').change(function() {
				...
				if ($('#pw').val() !== "" && $('#pwChk').val() !== "") {
					...
					if (!regexPw.test(pw.value)) {
						$('#pwChkNotice').html('비밀번호는 영문 대소문자, 숫자, 특수문자 6~20자리로 입력해야합니다.');
		...
		document.getElementById("submitBtn").addEventListener(
				...
				function() {
					...
					if (!regexId.test(id.value) || id.value == "") {
					    ...
					    id.focus();
					    return;
					}else if ($('#pw').attr("check_result") == "fail"){
